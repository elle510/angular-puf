...
function HideEmptyRows() {
    var d = document.getElementsByName("rowLink");
    var b;
    var c = document.getElementById("tbl1");
    var a = c.rows.length;
    for (b = a - 1; b != 0; b--) {
        if (c.rows[b].getAttribute("name") == "rowLink") {
            if (c.rows[b].getElementsByTagName("td")[0].innerHTML.replace(/^\s+|\s+$/g, "") == "") {
                c.deleteRow(b + 1);
                c.deleteRow(b)
            }
        }
...
function alternate(f) {
    ...
    if (document.getElementsByTagName) {
        ...
        for (i = 1; i < e.length; i++) {
            if (b == "odd") {
                if ((a * 1) < (i * 1) && i < e.length - 3) {
                    e[i].className = "even";
                    a = 1 * i;
                    if ((i + 1) <= e.length && i < e.length - 3) {
                        a = 1 * (i + 1);
                        e[a].className = "even";
                        b = "even"
                    }
                ...
                if ((a * 1) < (i * 1) && i < e.length - 3) {
                    e[i].className = "odd";
                    a = 1 * i;
                    if (i != 1 && (i + 1) <= e.length && i < e.length - 3) {
                        a = 1 * (i + 1);
                        e[a].className = "odd";
                        b = "odd"
                    }
...
function noFrms() {
    var d = parent.frames.classFrame.location.href.substring(window.location.href.lastIndexOf("/") + 1);
    if (d == "search.html") {
        var c = window.top.location.href.split("?");
        var a;
        if (c[1] != null) {
            c = c[1].split("#");
            var b = c[1];
...
function searchfunctionsubmit(b) {
...
function submitValue(g, e, d, c, f) {
    var g = g;
    var a = document.getElementById("search-livedocs").value;
    ...
    if (document.getElementById("checkBoxId").checked) {
        top.location.href = "http://" + e + "?q=" + escapeSpecialChars(a) + "&loc=" + f + "&hl=" + f + "&lbl=" + d + "&go=Search&self=1&site=" + c
    } else {
        top.location.href = "http://" + e + "?q=" + escapeSpecialChars(a) + "&loc=" + f + "&hl=" + f + "&lbl=" + d + "&go=Search&self=1"
    }
    document.getElementById("subTitle").childNodes.item(0).data = "";
    return false
}

function escapeSpecialChars(a) {
    a = a.replace(/%/g, "%25");
    a = a.replace(/!/g, "%21");
    a = a.replace(/@/g, "%40");
    a = a.replace(/#/g, "%23");
...
function addIonComments(b) {
    ...
    ionComAddLoadEvent(function() {
        var e = encodeURIComponent(window.location);
        var f = e.replace(new RegExp(/%23(.*)/), "");
        ionComments.setup(f, d, c, {
            siteArea: "help",
            productLabel: +b
        })
    })
...
function configPage() {
    var g = document.location.href.toString();
    parameter_index = g.lastIndexOf("?");
    var a = "";
    if (parameter_index != -1) {
        a = g.substring(parameter_index + 1, g.length)
    }
    if (a.indexOf("#") != -1) {
        a = a.replace(new RegExp(/#/), "%23")
    } else {
        if (a.indexOf("%23") != -1) {
            a = a.replace(new RegExp(/%23/), "#")
        }
    ...
    if (isEclipse()) {
        if (window.name != "classFrame") {
            var c = window.location.href.indexOf("?") != -1 ? window.location.href.substring(0, window.location.href.indexOf("?")) : window.location.href;
            var b = "doc/";
    ...
    try {
        getFilterContent(filter_file, a)
    } catch (f) {}
...
function loadFrames(d, b) {
    var c = findObject("classListFrame");
    if (c != null && classListFrameContent != "") {
        c.document.location.href = classListFrameContent
    }
    if (isEclipse()) {
        var a = findObject(ECLIPSE_FRAME_NAME);
        if (a != null && d != "") {
            a.document.location.href = d
        }
    } else {
        var e = findObject("classFrame");
        if (e != null && classFrameContent != "") {
            e.document.location.href = classFrameContent
        }
    }
}
...
function loadClassFrame(a) {
    if (parent.frames.classFrame != null) {
        parent.frames.classFrame.location = a
    } else {
        if (parent.frames.packageFrame != null) {
            if (parent.frames.packageFrame.frames.classFrame != null) {
                parent.frames.packageFrame.frames.classFrame.location = a
            }
        }
    }
}

function loadClassListFrame(a) {
    if (parent.frames.classListFrame != null) {
        parent.frames.classListFrame.location = a
    } else {
        if (parent.frames.packageFrame != null) {
            if (parent.frames.packageFrame.frames.classListFrame != null) {
                parent.frames.packageFrame.frames.classListFrame.location = a
            }
        }
    }
}
...
function loadPage(a) {
    document.location.href = baseRef + a
}

function gotoLiveDocs(b, d, a) {
    if (a == "en-us" || a == "en_us" || a == "en_US") {
        a = ""
    } else {
        a = "_" + a.substring(3)
    }
...
function findTitleTableObject(a) {
    if (isEclipse()) {
        return parent.titlebar.document.getElementById(a)
    } else {
        if (top.titlebar) {
            return top.titlebar.document.getElementById(a)
        } else {
            return document.getElementById(a)
        }
    }
...
function titleBar_setSubTitle(a) {
    if (isEclipse() || top.titlebar) {
        findTitleTableObject("subTitle").childNodes.item(0).data = a
    }
}

function titleBar_setSubNav(r, k, a, o, b, q, c, g, e, m, d, p, n, l, f, h) {
    findTitleTableObject("propertiesLink").style.display = k ? "inline" : "none";
...
function titleBar_gotoClassFrameAnchor(a) {
    if (isEclipse()) {
        parent.classFrame.location = parent.classFrame.location.toString().split("#")[0] + "#" + a
    } else {
        location = location.toString().split("#")[0] + "#" + a
    }
}
...
function setInheritedVisible(a, c) {
    try {
        if (document.styleSheets[0].cssRules != undefined) {
            var f = document.styleSheets[0].cssRules;
            for (var b = 0; b < f.length; b++) {
                if (f[b].selectorText == ".hideInherited" + c) {
                    f[b].style.display = a ? "" : "none"
                }
                if (f[b].selectorText == ".showInherited" + c) {
                    f[b].style.display = a ? "none" : ""
                }
            }
        } else {
            document.styleSheets[0].addRule(".hideInherited" + c, a ? "display:inline" : "display:none");
...
function setRowColors(a, d) {
    var f = "#F2F2F2";
    var c = findObject("summaryTable" + d);
    if (c != null) {
        var e = 0;
        for (var b = 1; b < c.rows.length; b++) {
            if (c.rows[b].className.indexOf("hideInherited") == -1 || a) {
                if (c.rows[b].style.display.indexOf("none") == -1) {
                    e++;
...
function setRowColorsInitial(a, d) {
    var f = "#F2F2F2";
    var c = findObject("summaryTable" + d);
    if (c != null) {
        var e = 0;
...
function setStyle(c, b, f) {
    try {
        if (document.styleSheets[0].cssRules != undefined) {
            var g = document.styleSheets[0].cssRules;
            for (var a = 0; a < g.length; a++) {
                if (g[a].selectorText == c) {
                    g[a].style[b] = f;
                    break
                }
            }
        } else {
            document.styleSheets[0].addRule(c, b + ":" + f)
        }
    } catch (d) {
        document.styleSheets[0].addRule(c, b + ":" + f)
    }
...
function loadPackagesList(d) {
    document.getElementById("packagelist").innerHTML = d.responseText;
    if (buildType != "jslr") {
...
function loadClassesList(f) {
    if (buildType == "jslr") {
        document.getElementById("classlist").innerHTML = f.responseText
    } else {
        document.getElementById("classlistoutline").innerHTML = f.responseText;
        ...
        if (getCookie("showAllClasses") == "yes") {
            ...
            if (a && a.style.display != "none" && a.value != a.defaultValue && a.value != "") {
...
function loadClassesList2(b) {
    document.getElementById("classlistWrapper").innerHTML = b.responseText;
    classesLoaded = true;
    if (isTablet()) {
        new iScroll("classlist");
        contentScroller = new iScroll("content")
...
function isTablet() {
    ...
    if (/ipad/i.test(a) || /android/i.test(a) || /iphone/i.test(a) || /PlayBook/i.test(a)) {
        return true
    } else {
        return false
    }
...
function init1() {
    ...
    if (buildType.indexOf("jslr") != -1) {
        ...
        if (location.href.indexOf("?allClasses=1") > 0) {
            ...
            if ((location.href.indexOf("mxml-tag-detail.html") > 0) || (baseRef == "../" && (location.href.indexOf("/mxml/") > 0))) {
                ajaxGet(baseRef + "mxml-tags.html", loadClassesList2)
            } else {
                if ((baseRef == "") && (location.href.indexOf("package-detail.html") > 0 || location.href.indexOf("package.html") > 0)) {
                    ...
                    if (baseRef == "" || baseRef == "./") {
                        ...
                        if (location.href.indexOf("package-detail") > 0) {
                            ajaxGet("class-list.html", loadClassesList2)
                        } else {
                            if (getCookie("showAllClassesList") == "yes") {
                                if (getCookie("showAllClasses") == "yes") {
                                    ajaxGet(baseRef + "all-classes.html", loadClassesList)
                                } else {
                                    ajaxGet(baseRef + "all-classes-A.html", loadClassesList)
...
function usingPushState() {
    if ("pushState" in window.history) {
        if ($.browser.safari) {
            ...
            if (typeof(a[0]) != "undefined" && a[0] > 533) {
                return true
            } else {
                if (typeof(a[0]) != "undefined" && a[0] == 533) {
                    if (typeof(a[1]) != "undefined" && a[1] > 18) {
                        return true
                    } else {
                        return false
                    }
...
function init2() {
    ...
    document.getElementById("maincontainer").style.display = "";
    if ($.browser.msie || agt.indexOf("adobe help viewer 2") != -1 || agt.indexOf("community help client") != -1 || location.protocol.match(/file:/)) {
        ...
        $(document).bind("contextmenu", function(d) {
            rightClickHandler(d)
        });
        if (usingPushState()) {
            if (unescape(location.href).indexOf("#!") > 0) {
        ...
        if (unescape(location.href).match(/\?/)) {
            var c = unescape(location.href).split("?")[0];
            var b = unescape(location.href).split("?")[1];
            if (b.match(/nameAnchor/i)) {
                var a = b.split("nameAnchor=")[1];
                location.href = c + "#" + a
            } else {
                loadFilters(b);
                if (b.match(/\#/)) {
...
function init3() {
    ...
    try {
        if (!usingPushState() && location.href.match(/#!/)) {} else {
            loadIonComments()
        }
    } catch (a) {}
}
...
function getURLBaseLocation(a) {
    a = unescape(a);
    a = getURLFullLocation(a);
    if (a.match(/langref\//)) {
        a = a.substring(0, a.indexOf("langref/") + 8)
    } else {
        if (a.match(/actionscript\/3\//)) {
            a = a.substring(0, a.indexOf("actionscript/3/") + 15)
        }
    }
    return a
}

function getAjaxFullFragment(a) {
    a = unescape(a);
...
function getAjaxBaseFragment(a) {
    a = unescape(a);
...
function getLangrefFullFragment(a) {
    a = unescape(a);
    a = a.split("#!")[0];
    a = a.split("#")[0];
    if (a.match(/langref/)) {
        a = a.substring(a.indexOf("langref/") + 8)
    } else {
        if (a.match(/actionscript\/3/)) {
            a = a.substring(a.indexOf("actionscript/3/") + 15)
        }
    }
    return a
}

function getLangrefBaseFragment(b) {
    b = unescape(b);
...
function getHyperlinkFragment(a) {
    var b = a.attr("href");
    if (!isExternalLink(b)) {
        if (b.match(/\.\.\//)) {
            b = b.replace(/\.\.\//g, "")
...
function rightClickHandler(f) {
    AsDocs.log("handling right click");
    if ($(f.target)[0].nodeName.toLowerCase() == "a" || $(f.target).parents("a")[0]) {
        var h = "";
        if ($(f.target).parents("a").length > 0) {
            h = $(f.target).parents("a")
        } else {
            h = $(f.target)
        }
        if (isExternalLink(h.attr("href"))) {
            return true
        }
        if (typeof(h.data("changedFromHref")) != "undefined" && h.data("changedFromHref") != "") {
            return true
        }
        if (usingPushState()) {
            if ($(f.target).parents(".titleTableTitleION").length) {
                var e = getHyperlinkFragment(h);
                h.data("changedFromHref", a);
                h.attr("href", getBaseRef() + e)
            }
        } else {
            var b = getURLFullLocation(location.href);
            var c = getAjaxBaseFragment(location.href);
            var g = getAjaxFullFragment(location.href);
            var d = getLangrefBaseFragment(location.href);
            var a = h.attr("href");
            var e = getHyperlinkFragment(h);
            if ($(f.target).parents("#toc").length) {
                if ($(f.target).parents("#classlist").length > 0 && $(f.target).parents("#classlistoutline").length <= 0) {
                    if (g.match(/^mxml\//)) {
                        h.data("changedFromHref", a);
                        h.attr("href", getBaseRef() + e)
                    } else {
            ...
            if ($(f.target).parents("#content").length) {
                if (g.match(/package-detail/)) {
                    h.data("changedFromHref", a);
                    h.attr("href", getBaseRef() + c + e)
                } else {
                    if (e.match(/package-detail/)) {
                        if (c == "") {
                            h.data("changedFromHref", a);
                            h.attr("href", getBaseRef() + d + e)
                        } else {
                        ...
                        if (b.match(/package-detail/) && g == "") {
                            h.data("changedFromHref", a);
                            h.attr("href", getBaseRef() + d + e)
                        } else {
                            if (h.attr("href").match(/\//)) {
                                h.data("changedFromHref", a);
                                h.attr("href", getBaseRef() + e)
                            } else {
                                if (c == "") {
            ...
            if ($(f.target).parents(".titleTableSubTitle").length) {
                if (c == "") {
                    h.data("changedFromHref", a);
                    h.attr("href", getBaseRef() + d + e)
                } else {
...
function clickHandler(b) {
    ...
    var o = b;
    if (b.which == "3" || b.which == "2" || b.ctrlKey == true) {
        rightClickHandler(b);
        return true
    }
    if (b.target.nodeName.toLowerCase() == "input") {
        return true
    }
    if ($(b.target).parents("#filter_panel_float").length) {
        return true
    }
    if ($(b.target).parents("#badgeContainerSupport").length) {
        return true
    }
    if ($(b.target).parents("#ionComHere").length) {
        return true
    }
    if ($(b.target).parents("#creativecommons").length > 0) {
        return true
    }
    if ($(b.target).parents("#whyEnglish").length > 0 || $(b.target).attr("id") == "whyEnglish") {
        return true
    }
    if ($(b.target).parents("#whyEnglishContent").length > 0 || $(b.target).attr("id") == "whyEnglishContent") {
        return true
    }
    if ($(b.target).parents("a").length > 0) {
        if (isExternalLink($(b.target).parents("a").attr("href"))) {
            return true
        }
    } else {
        if (isExternalLink($(b.target).attr("href"))) {
            return true
        }
    }
    b.preventDefault ? b.preventDefault() : b.returnValue = false;
    if ($(b.target).parents(".loadingErrorDiv").length) {
        if ($(b.target).attr("class") == "innerLoadingErrorSpan2") {
            clearRightPane();
            loadAjaxPage(lastHref)
        }
        if ($(b.target).attr("class") == "innerLoadingErrorSpan3") {
            if (typeof lastRequest == "undefined" || lastRequest == "") {} else {
                lastRequest.abort()
            }
            clearRightPane()
        }
    }
    if ($(b.target).parents(".loadingMsgDiv").length) {
        if ($(b.target).attr("class") == "innerLoadingMsgSpan2") {
            if (typeof lastRequest == "undefined" || lastRequest == "") {} else {
                lastRequest.abort()
            }
            clearRightPane()
    ...
    if (usingPushState()) {
        ...
        if ($(b.target)[0].nodeName.toLowerCase() == "a" || $(b.target).parents("a")[0]) {
            var g = "";
            if ($(b.target).parents("a").length > 0) {
                g = $(b.target).parents("a")
            } else {
                g = $(b.target)
            }
            if (typeof g.data("changedFromHref") != "undefined") {
                g.attr("href", g.data("changedFromHref"));
                g.data("changedFromHref", null)
            }
            var c = getHyperlinkFragment(g);
            if ($(b.target).parents(".titleTableTitleION").length) {
                AsDocs.log("top left links matched");
                prevURL = unescape(location.href);
                if ($(b.target).attr("id") == "hidetoc" || $(b.target).attr("id") == "showtoc") {} else {
                    window.history.pushState("", "", getBaseRef() + c);
                    curURL = unescape(location.href);
                    loadAjaxPage(location.href)
                }
                return false
            }
            if (g.attr("href").match(/^#/)) {
            ...
            if ($(b.target).parents("#toc").length) {
                AsDocs.log("toc links matched");
                var l;
                l = getBaseRef();
                prevURL = unescape(location.href);
                if ($(b.target).parents("#packagelist").length) {
                    window.history.pushState("", "", l + c)
                }
                if ($(b.target).parents("#classlistoutline").length) {
                    window.history.pushState("", "", l + c)
                } else {
                    if ($(b.target).parents("#classlist").length) {
            ...
            if ($(b.target).parents("#content").length) {
                ...
                if (g.attr("href").match(/\?/)) {
                    ...
                    window.history.pushState("", "", g.attr("href"));
                    curURL = unescape(location.href);
                    var e = prevURL.match(/#/) ? prevURL.split("#")[0] : prevURL;
                    var d = curURL.match(/#/) ? curURL.split("#")[0] : curURL;
                    if (e == d) {
            ...
            if ($(b.target).parents(".titleTableSubTitle").length) {
        ...
        if ($(b.target)[0].nodeName.toLowerCase() == "a" || $(b.target).parents("a")[0]) {
            ...
            if ($(b.target).parents(".titleTableTitleION").length > 0) {
                prevURL = unescape(location.href);
                if (g.attr("href").match(/#top/)) {} else {
                    location.href = k + "#!" + c
                }
                curURL = unescape(location.href);
                return false
            ...
            if (g.attr("href").match(/^#/)) {
                prevURL = unescape(location.href);
                if (n == "") {
                    location.href = k + "#" + getScrollTarget(c)
                } else {
                    location.href = k + "#!" + n + "#" + getScrollTarget(c)
                }
                curURL = unescape(location.href);
                if (prevURL == curURL) {
                    URLHashChange()
            ...
            if ($(b.target).parents("#toc").length) {
                prevURL = unescape(location.href);
                if ($(b.target).parents("#classlist").length > 0 && $(b.target).parents("#classlistoutline").length <= 0) {
                    if (c.match(/mxml\//)) {
                        location.href = k + "#!" + c
                    } else {
                        if (n == "") {
            ...
            if ($(b.target).parents("#content").length) {
                ...
                if (g.attr("href").match(/\.\.\//)) {
                    ...
                    if (c.match(/package-detail/)) {
                        if (n.match(/package-summary/) || n.match(/class-summary/)) {
                            location.href = k + "#!" + c
                        } else {
                            if (f == "") {
                                location.href = k + "#!" + a + c
                        ...
                        if (k.match(/package-detail/) && n == "") {
                            location.href = k + "#!" + a + c
                        } else {
                            if (f == "") {
                                if (n == "package-detail.html") {
                                    location.href = k + "#!" + c
                                } else {
...
function popHistory() {
    ...
    if (prevURL == "") {} else {
        if (prevURL == curURL) {} else {
            if (prevURL.split("#")[0] == curURL.split("#")[0]) {
...
function URLHashChange() {
    ...
    if (f.split("#!").length == 1) {
        if (prevURL == "") {} else {
            if (prevURL.split("#")[0] == curURL.split("#")[0] && curURL.split("#").length > 1) {
                scrollHere(curURL.split("#")[1])
            } else {
                loadAjaxPage(f)
            }
        ...
        if (f.match(/\?/)) {
            ...
            if (f.split("#!").length == 2 && f.split("#").length != 3) {
                AsDocs.log("3");
                ...
                if (f.split("#").length == 3) {
                    ...
                    if (d == c) {
                        ...
                        loadAjaxPage(a + c, curURL.split("#")[2])
                    }
                }
            }
        }
...
function handleClassListLoading(g, e) {
    ...
    if (usingPushState()) {
        ...
        var n = getAjaxFullFragment(g);
        var k = getAjaxBaseFragment(g);
        var l = getAjaxFullFragment(e);
        var b = getAjaxBaseFragment(e);
        var d = getURLBaseLocation(location.href);
        if ($("#classlistoutline").length <= 0) {
            if (l.match(/package-detail/)) {
                if (l == n) {
                    ajaxGet(d + b + "class-list.html", loadClassesList2)
                } else {
                    if (k == b) {
                        if (!n.match(/package-detail/) && l.match(/package-detail/)) {
                            ajaxGet(d + b + "class-list.html", loadClassesList2)
                        }
                    } else {
                        ajaxGet(d + b + "class-list.html", loadClassesList2)
                ...
                if (l.match(/mxml-tag-detail/) || l.match(/mxml\//)) {
                    ...
                    if (n.match(/mxml-tag-detail/)) {
                        ...
                        if (l == "language-elements.html" || l == "operators.html" || l == "statements.html" || l == "specialTypes.html" || l == "appendixes.html" || l == "compilerErrors.html" || l == "compilerWarnings.html" || l == "runtimeErrors.html" || l == "charset-codes.html" || l == "motionXSD.html" || l == "TimedTextTags.html" || l == "deprecated.html" || l == "accessibilityImplementationConstants.html" || l == "index.html" || l == "package-summary.html" || l == "class-summary.html" || l == "all-index-Symbols.html" || l == "appendixes.html") {
                            ...
                            if (l == "" && unescape(location.href).match(/package-detail/)) {
                                ajaxGet("class-list.html", loadClassesList2)
                            } else {
                                if (b != k) {
                                    if (l == "") {
                                        ajaxGet(d + getLangrefBaseFragment(e) + "class-list.html", loadClassesList2)
                                    } else {
                                        ajaxGet(d + b + "class-list.html", loadClassesList2)
                                    }
                                }
...
function createLoadingMsg() {
    $("#maincontainer").append("<div class='loadingMsgDiv'><div class='innerLoadingMsgDiv'><span class='innerLoadingMsgSpan1'>" + ajaxLoadingMsg + "... </span><span class='innerLoadingMsgSpan2'>" + cancelMsg + "</span></div></div>")
}
...
function scrollHere(a) {
    var d = encodeURIComponent(a);
    try {
        if (isTablet()) {
            contentScroller.scrollToElement(document.getElementById(d));
            return
        }
    ...
    $("#content img").each(function() {
        ...
        if (usingPushState()) {
            ...
            if ($.browser.msie && $.browser.version <= 7) {
                var a = imgsrc.substring(imgsrc.indexOf("images"));
...
function updateOmniture(b) {
    if (typeof(s_accountName) != "undefined") {
        var a = s_gi(s_accountName);
...
function getIonComments() {
    label = getIonLabel();
    site = getIonSite();
    if (document.getElementById("ionComHere") || document.getElementById("badgeContainerSupport")) {
...
function isHomepage() {
    var b = document.all ? document.all.tags("meta") : document.getElementsByTagName ? document.getElementsByTagName("meta") : new Array();
    for (var a = 0; a < b.length; a++) {
        if ((b[a].name == "topictype") && (b[a].content == "homepage")) {
            return true
        }
    }
    return false
...
function getIonLabel() {
    ...
    if (a.indexOf("Acrobat/9.0/Reviewer") != -1) {
        return b
    }
...
function scrollToNameAnchor() {
    ...
    if (b[1] != null) {
        document.location = b[0] + "#" + b[1]
    }
}

function ajaxGet(b, a) {
    var d = false;
...
function h_splitterMouseDown(a, b) {
...
function h_splitterMouseUp(b) {
    if (h_splitterActiveFlag) {
        var a = document.getElementById("packagelist");
...
function h_splitterMouseMove(a) {
    if (h_splitterActiveFlag) {
        var b = a.clientY - document.getElementById("maincontainer").offsetTop;
        if (b >= MIN_TOP && a.clientY <= document.documentElement.clientHeight - MIN_BOTTOM) {
            h_splitterObj.style.top = b + "px";
            if (!h_splitterObj.releaseCapture) {
                a.preventDefault()
            }
        }
...
function setStyle(c, f, b, e) {
    var a = false;
    try {
        if (c.style && c.style.setProperty) {
            c.style.setProperty(f, b, e);
            a = true
        }
    } catch (d) {
        alert("exception caught setting style: " + d.message)
    }
    if (!a) {
        try {
            c.style[f] = b;
            ...
            alert("exception caught setting direct style: " + g.message)
        }
    }
    return a
}
...
function savePackageListScrollTop() {
    var a = document.getElementById("packagelist");
    if (a) {
        setCookie("packageListScrollTop", a.scrollTop, new Date(3000, 1, 1, 1, 1), "/", document.location.domain);
        setCookie("classListScrollTop", 0, new Date(3000, 1, 1, 1, 1), "/", document.location.domain)
    }
}

function saveClassListScrollTop() {
    var a = document.getElementById("classlist");
    if (a) {
        setCookie("classListScrollTop", a.scrollTop, new Date(3000, 1, 1, 1, 1), "/", document.location.domain)
    }
}

function restorePackageListScrollTop() {
    var a = getCookie("packageListScrollTop");
    if (a) {
        document.getElementById("packagelist").scrollTop = a
    }
}

function restoreClassListScrollTop() {
    var a = getCookie("classListScrollTop");
    if (a) {
        document.getElementById("classlist").scrollTop = a
    }
}
...
function restoreSplitterPos() {
    var a = getCookie("splitterPosition");
    if (a) {
        document.getElementById("splitter").style.left = a + "px"
    }
}

function saveHSplitterPos() {
    var a = document.getElementById("h_splitter");
    if (a) {
        setCookie("h_splitterPosition", a.offsetTop, new Date(3000, 1, 1, 1, 1), "/", document.location.domain)
    }
}
...
function restoreHSplitterPos() {
    var a = document.getElementById("h_splitter");
    if (a) {
        var b = getCookie("h_splitterPosition");
        if (b) {
            a.style.top = b + "px"
        }
    }
...
function setHrfJSLR(a) {
    var b = a.title;
    b = b.replace(/\./g, "/");
    if (b.search("\\(\\)") > 0) {
        titleSub = b.substr(b.lastIndexOf("/") + 1);
...
function quickFilterPackages() {
    var b = document.getElementById("pkg_searchbox").value;
    ...
    for (i = 0; i < h.length; i++) {
        var d;
        if (h[i].textContent) {
            d = h[i].textContent.match(g)
        } else {
            d = h[i].innerText.match(g)
        }
        if (h[i].className == "noqsearch") {
            h[i].style.display = "none"
        } else {
            if (h[i].id == "pkg_name") {} else {
                if (d != null && d.length > 0 && h[i].parentNode.parentNode.style.display != "none") {
                    k++;
                    var a = h[i].innerHTML;
                    h[i].innerHTML = a.replace(d[0], "<span class='highlightText'>" + d[0] + "</span>")
                } else {
                    if (h[i].id == "pkg_name" || h[i].id == "clear_search") {} else {
                        h[i].style.display = "none"
                    }
                }
            }
    ...
    if (k == 0) {
        var f = document.getElementById("pkg_txtMsg");
        if (f == null) {
...
function pkgSearchBoxOnFocus() {
    var a = document.getElementById("pkg_searchbox");
...
function pkgClearHighlight() {
    ...
    if (navigator.userAgent.search("MSIE") >= 0) {
        xtemp = a.getElementsByTagName("span");
        j = 0;
        ...
        x = a.getElementsByClassName("highlightText");
        while (x.length > 0) {
            y = x[0].parentNode;
            z = x[0].innerHTML;
            txtNode = document.createTextNode(z);
...
function quickFilterClassList() {
    var b = document.getElementById("cls_searchbox").value;
    ...
    for (i = 0; i < h.length; i++) {
        var d;
        if (h[i].textContent) {
            d = h[i].textContent.replace(/^\s\s*/, "").replace(/\s\s*$/, "").match(f)
        } else {
            d = h[i].innerText.replace(/^\s\s*/, "").replace(/\s\s*$/, "").match(f)
        }
        if (h[i].className == "noqsearch") {
            h[i].style.display = "none"
        } else {
            if (d != null && d.length > 0 && h[i].style.display != "none") {
                g++;
                var a = h[i].innerHTML;
                h[i].innerHTML = a.replace(d[0], "<span class='clsHighlightText'>" + d[0] + "</span>")
            } else {
                if (h[i].id == "cls_name" || h[i].id == "cls_clear_search") {} else {
                    h[i].style.display = "none"
                }
            }
        }
...
function clsClearHighlight() {
    var a = document.getElementById("classlistoutline");
    x = [];
    var c = document.getElementById("cls_txtMsg");
    ...
    if (navigator.userAgent.search("MSIE") >= 0) {
        ...
        x = a.getElementsByClassName("clsHighlightText");
        while (x.length > 0) {
            y = x[0].parentNode;
            z = x[0].innerHTML;
            txtNode = document.createTextNode(z);
